...
function drawWingMark(g: CanvasRenderingContext2D, cx: number, cy: number, s: number, color: string) {
  g.save()
  g.translate(cx, cy)
  g.fillStyle = color
  g.globalAlpha = 0.92
...
function sideTexture() {
  ...
  drawWingMark(g, cv.width / 2, 130, 26, INK)
...
function topTexture() {
  ...
  drawWingMark(g, cv.width / 2, 256, 22, 'rgba(23,34,63,.5)')
